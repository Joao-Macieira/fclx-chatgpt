...
export default function Home() {
  ...
  const { data: chats, mutate: chatsMutate } = useSWR<ChatWithFirstMessage[]>('chats', fetcher, {
    fallbackData: [],
    revalidateOnFocus: false,
  });

  const { data: messages, mutate: messagesMutate } = useSWR<Message[]>(chatId ? `chats/${chatId}/messages` : null, fetcher, {
    fallbackData: [],
    revalidateOnFocus: false,
  });

  const { data: messageLoading, error: errorMessageLoading } = useSWRSubscription(messageId ? `/api/messages/${messageId}/events` : null, (path: string, { next }) => {
    ...
    eventSource.addEventListener('end', (event) => {
      eventSource.close();
      const newMessage = JSON.parse(event.data);
      messagesMutate((messages) => [...messages!, newMessage], false);
      next(null, null);
    });
